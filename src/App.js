...
export const App = ({login, logout, notify, filter, resetRides, rides, refreshing, fresh, user}) => {
  ...
  useEffect(
    ...
    [user, userManager, propagateUser]
  )
  ...
  useEffect(
    () => {
      if (!fresh && window.location.pathname !== '/callback') {
        refreshing()
